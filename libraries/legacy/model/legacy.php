...
abstract class JModelLegacy extends JObject
{
	...
	public function loadHistory($version_id, JTable &$table)
	{
		...
		if ($version_id)
		{
			...
			{
				$typeAlias = JFactory::getApplication()->input->getCmd('extension') . '.category';
			}
			else
			{
				$typeAlias = $this->option . '.' . $this->name;
			}
			$typeId = JTable::getInstance('Contenttype')->getTypeId($typeAlias);
